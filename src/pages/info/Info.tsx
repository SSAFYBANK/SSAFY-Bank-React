...
function Info() {
    //예금주 전체 가져오기
    const [data, setData] = useState<Array<{ accountHolderName: string, accountHolderUuid: string }>>([]);
    async function fetchData() {
        try {
          const token = sessionStorage.getItem("Authorization");
          const response = await axios.get("http://52.78.102.165:8081/v1/account-holder/getList", {
            headers: {
    ...
    useEffect(() => {
      ...
      async function fetchAccountDetails(uuid: string) {
        try {
          const token = sessionStorage.getItem("Authorization");
          const response = await axios.get(`http://52.78.102.165:8081/v1/account/getHolderAccountList/${uuid}/0`, {
            headers: {
              'Authorization': token
            }
          });
...
const [transferHistory, setTransferHistory] = useState<Array<{
  ...
  function formatDateTime(dateTimeStr : any) {
            ...
            <Container>
                {data.map((item, index) => (
                    ...
                    <AccountHolderTop>
                        ...
                        <DetailsDiv onClick={() => handleShowCreateModal(item.accountHolderUuid)}>계좌등록</DetailsDiv>
                        <DetailsDiv onClick={() => handleShowDeleteModal(item.accountHolderUuid, item.accountHolderName)}>예금주삭제</DetailsDiv>
                        <DetailsDiv onClick={() => toggleDropdown(item.accountHolderUuid)}>계좌현황</DetailsDiv>
                    </AccountHolderTop>
                    {dropdownStatus[item.accountHolderUuid] && (
                        ...
                        {accountDetails[item.accountHolderUuid]?.map((account, i) => (
                            ...
                            <Label>계좌번호 : </Label><Data>{account.accountNum}</Data>
                            <Label>잔액 : </Label><Data>{account.balance} 원</Data>
                            <DetailsDiv onClick={() => handleShowTransferModal(account.accountNum,item.accountHolderUuid)}>이체하기</DetailsDiv>
      <DetailsDiv onClick={() => handleShowDeleteAccountModal(account.accountNum,item.accountHolderUuid)}>계좌삭제</DetailsDiv>
      <DetailsDiv onClick={() => handleShowTransactionHistoryModal(account.accountNum)}>거래내역</DetailsDiv>
                            </AccountInfoOne>
                        ))}
                        </AccountInfo>
                    )}
            ...
            <Modal2 show={showModal2} onClose={() => setShowModal2(false)}>
                ...
                <InputBox type="text" value={bankCode} onChange={(e) => setBankCode(e.target.value)} />
            </div>

            <div>
                <label>패스워드(4자리):</label>
                <InputBox type="password" value={accountPassword} onChange={(e) => setAccountPassword(e.target.value)} />
            </div>
            </WrapModal2>
                <WarpBtnModal>
            ...
            <div>
                <label>계좌번호:</label>
                <InputBox type="text" value={accountNum} onChange={(e) => setAccountNum(e.target.value)} />
            </div>
            <div>
                <label>이체금액:</label>
                <InputBox type="text" value={amount} onChange={(e) => setAmount(e.target.value)} />
            </div>
            <div>
                <label>입금통장인자(상대방 계좌에 남음):</label>
                <InputBox type="text" value={receiverContent} onChange={(e) => setReceiverContent(e.target.value)} />
            </div>
            <div>
                <label>출금통장인자(내 계좌에 남음):</label>
                <InputBox type="text" value={senderContent} onChange={(e) => setSenderContent(e.target.value)} />
            </div>
            <div>
                <label>패스워드(4자리):</label>
                <InputBox type="password" value={accountPassword} onChange={(e) => setAccountPassword(e.target.value)} />
            ...
            <Modal4 show={showTransactionHistoryModal} onClose={() => setShowTransactionHistoryModal(false)}>
      ...
      {transferHistory.map((item, index) => (
        <TransactionItem key={index}>
          <div>
            ...
            <TransactionData>{formatDateTime(item.transferDate)}</TransactionData>
          </div>
          <div>
    ...
    <DetailsDiv2 onClick={() => setShowTransactionHistoryModal(false)}>취소</DetailsDiv2>
                </WarpBtnModal>
